...
describe('Jira Commands — missing config', () => {
  ...
  it('get-issue returns error without throwing', async () => {
    const result = await getIssue.handler(ctx({ issueKey: 'TEST-1' }));
    expect(result.exitCode).toBe(1);
    expect(result.output).toBeTruthy();
  });

  it('search returns error without throwing', async () => {
    const result = await search.handler(ctx({ jql: 'project=TEST' }));
    expect(result.exitCode).toBe(1);
  });

  it('get-project-issues returns error without throwing', async () => {
    const result = await getProjectIssues.handler(ctx({ projectKey: 'TEST' }));
    expect(result.exitCode).toBe(1);
  });

  it('create-issue returns error without throwing', async () => {
    const result = await createIssue.handler(ctx({ projectKey: 'TEST', issueType: 'Task', summary: 'Test' }));
    expect(result.exitCode).toBe(1);
  });

  it('update-issue returns error without throwing', async () => {
    const result = await updateIssue.handler(ctx({ issueKey: 'TEST-1', fields: { summary: 'x' } }));
    expect(result.exitCode).toBe(1);
  });

  it('delete-issue returns error without throwing', async () => {
    const result = await deleteIssue.handler(ctx({ issueKey: 'TEST-1' }));
    expect(result.exitCode).toBe(1);
  });

  it('batch-create-issues returns error without throwing', async () => {
    const result = await batchCreateIssues.handler(ctx({ issues: [{ fields: { summary: 'A' } }] }));
    expect(result.exitCode).toBe(1);
  });

  it('get-changelogs returns error without throwing', async () => {
    const result = await getChangelogs.handler(ctx({ issueKey: 'TEST-1' }));
    expect(result.exitCode).toBe(1);
  });
  ...
  it('get-field-options returns error without throwing', async () => {
    const result = await getFieldOptions.handler(ctx({ fieldId: 'cf_10001', contextId: '10000' }));
    expect(result.exitCode).toBe(1);
  });

  it('add-comment returns error without throwing', async () => {
    const result = await addComment.handler(ctx({ issueKey: 'TEST-1', body: 'hello' }));
    expect(result.exitCode).toBe(1);
  });

  it('edit-comment returns error without throwing', async () => {
    const result = await editComment.handler(ctx({ issueKey: 'TEST-1', commentId: '10000', body: 'x' }));
    expect(result.exitCode).toBe(1);
  });

  it('get-transitions returns error without throwing', async () => {
    const result = await getTransitions.handler(ctx({ issueKey: 'TEST-1' }));
    expect(result.exitCode).toBe(1);
  });

  it('transition-issue returns error without throwing', async () => {
    const result = await transitionIssue.handler(ctx({ issueKey: 'TEST-1', transitionId: '31' }));
    expect(result.exitCode).toBe(1);
  });
  ...
  it('get-project-versions returns error without throwing', async () => {
    const result = await getProjectVersions.handler(ctx({ projectKey: 'TEST' }));
    expect(result.exitCode).toBe(1);
  });

  it('get-project-components returns error without throwing', async () => {
    const result = await getProjectComponents.handler(ctx({ projectKey: 'TEST' }));
    expect(result.exitCode).toBe(1);
  });

  it('create-version returns error without throwing', async () => {
    const result = await createVersion.handler(ctx({ projectKey: 'TEST', name: 'v1.0' }));
    expect(result.exitCode).toBe(1);
  });

  it('batch-create-versions returns error without throwing', async () => {
    const result = await batchCreateVersions.handler(ctx({ projectKey: 'TEST', versions: [{ name: 'v1' }] }));
    expect(result.exitCode).toBe(1);
  });
  ...
  it('get-board-issues returns error without throwing', async () => {
    const result = await getBoardIssues.handler(ctx({ boardId: 1 }));
    expect(result.exitCode).toBe(1);
  });

  it('get-sprints-from-board returns error without throwing', async () => {
    const result = await getSprintsFromBoard.handler(ctx({ boardId: 1 }));
    expect(result.exitCode).toBe(1);
  });

  it('get-sprint-issues returns error without throwing', async () => {
    const result = await getSprintIssues.handler(ctx({ sprintId: 1 }));
    expect(result.exitCode).toBe(1);
  });

  it('create-sprint returns error without throwing', async () => {
    const result = await createSprint.handler(ctx({ boardId: 1, name: 'Sprint 1' }));
    expect(result.exitCode).toBe(1);
  });

  it('update-sprint returns error without throwing', async () => {
    const result = await updateSprint.handler(ctx({ sprintId: 1, name: 'Updated' }));
    expect(result.exitCode).toBe(1);
  });

  it('add-issues-to-sprint returns error without throwing', async () => {
    const result = await addIssuesToSprint.handler(ctx({ sprintId: 1, issueKeys: ['TEST-1'] }));
    expect(result.exitCode).toBe(1);
  });

  it('create-issue-link returns error without throwing', async () => {
    const result = await createIssueLink.handler(
  ...
  it('link-to-epic returns error without throwing', async () => {
    const result = await linkToEpic.handler(ctx({ epicKey: 'TEST-1', issueKeys: ['TEST-2'] }));
    expect(result.exitCode).toBe(1);
  });

  it('create-remote-issue-link returns error without throwing', async () => {
    const result = await createRemoteIssueLink.handler(
  ...
  it('remove-issue-link returns error without throwing', async () => {
    const result = await removeIssueLink.handler(ctx({ linkId: '10000' }));
    expect(result.exitCode).toBe(1);
  });

  it('get-worklog returns error without throwing', async () => {
    const result = await getWorklog.handler(ctx({ issueKey: 'TEST-1' }));
    expect(result.exitCode).toBe(1);
  });

  it('add-worklog returns error without throwing', async () => {
    const result = await addWorklog.handler(ctx({ issueKey: 'TEST-1', timeSpent: '2h' }));
    expect(result.exitCode).toBe(1);
  });

  it('download-attachment returns error without throwing', async () => {
    const result = await downloadAttachment.handler(ctx({ attachmentId: '10000' }));
    expect(result.exitCode).toBe(1);
  });

  it('get-issue-images returns error without throwing', async () => {
    const result = await getIssueImages.handler(ctx({ issueKey: 'TEST-1' }));
    expect(result.exitCode).toBe(1);
  });
  ...
  it('get-issue-watchers returns error without throwing', async () => {
    const result = await getIssueWatchers.handler(ctx({ issueKey: 'TEST-1' }));
    expect(result.exitCode).toBe(1);
  });

  it('add-watcher returns error without throwing', async () => {
    const result = await addWatcher.handler(ctx({ issueKey: 'TEST-1', accountId: '123' }));
    expect(result.exitCode).toBe(1);
  });

  it('remove-watcher returns error without throwing', async () => {
    const result = await removeWatcher.handler(ctx({ issueKey: 'TEST-1', accountId: '123' }));
    expect(result.exitCode).toBe(1);
  });
  ...
  it('get-service-desk-queues returns error without throwing', async () => {
    const result = await getServiceDeskQueues.handler(ctx({ serviceDeskId: 1 }));
    expect(result.exitCode).toBe(1);
  });

  it('get-queue-issues returns error without throwing', async () => {
    const result = await getQueueIssues.handler(ctx({ serviceDeskId: 1, queueId: 1 }));
    expect(result.exitCode).toBe(1);
  });

  it('get-issue-forms returns error without throwing', async () => {
    const result = await getIssueForms.handler(ctx({ issueKey: 'TEST-1' }));
    expect(result.exitCode).toBe(1);
  });

  it('get-form-details returns error without throwing', async () => {
    const result = await getFormDetails.handler(ctx({ issueKey: 'TEST-1', formId: 'form-1' }));
    expect(result.exitCode).toBe(1);
  });

  it('update-form-answers returns error without throwing', async () => {
    const result = await updateFormAnswers.handler(
  ...
  it('get-issue-dates returns error without throwing', async () => {
    const result = await getIssueDates.handler(ctx({ issueKey: 'TEST-1' }));
    expect(result.exitCode).toBe(1);
  });

  it('get-issue-sla returns error without throwing', async () => {
    const result = await getIssueSla.handler(ctx({ issueKey: 'TEST-1' }));
    expect(result.exitCode).toBe(1);
  });

  it('get-dev-info returns error without throwing', async () => {
    const result = await getDevInfo.handler(ctx({ issueId: '10001' }));
    expect(result.exitCode).toBe(1);
  });

  it('get-dev-summary returns error without throwing', async () => {
    const result = await getDevSummary.handler(ctx({ issueId: '10001' }));
    expect(result.exitCode).toBe(1);
  });

  it('get-batch-dev-info returns error without throwing', async () => {
    const result = await getBatchDevInfo.handler(ctx({ issueIds: ['10001'] }));
    expect(result.exitCode).toBe(1);
  });
});
...
describe.skipIf(!hasCredentials)('Jira Commands — live API', { timeout: 30_000 }, () => {
  ...
  it('search returns issues', async () => {
    const result = await search.handler(ctx({ jql: `project = ${testConfig.projectKey}`, maxResults: 5 }));
    expect(result.exitCode).toBe(0);
  });

  it('get-project-issues returns issues', async () => {
    const result = await getProjectIssues.handler(ctx({ projectKey: testConfig.projectKey, maxResults: 5 }));
    expect(result.exitCode).toBe(0);
  });

  it('get-issue returns issue details', async () => {
    const result = await getIssue.handler(ctx({ issueKey: testConfig.issueKey }));
    expect(result.exitCode).toBe(0);
  });

  it('get-transitions returns transitions', async () => {
    const result = await getTransitions.handler(ctx({ issueKey: testConfig.issueKey }));
    expect(result.exitCode).toBe(0);
  });

  it('get-changelogs returns changelog', async () => {
    const result = await getChangelogs.handler(ctx({ issueKey: testConfig.issueKey, maxResults: 5 }));
    expect(result.exitCode).toBe(0);
  });

  it('get-issue-watchers returns watchers', async () => {
    const result = await getIssueWatchers.handler(ctx({ issueKey: testConfig.issueKey }));
    expect(result.exitCode).toBe(0);
  });

  it('get-worklog returns worklog', async () => {
    const result = await getWorklog.handler(ctx({ issueKey: testConfig.issueKey }));
    expect(result.exitCode).toBe(0);
  });

  it('get-issue-images returns images', async () => {
    const result = await getIssueImages.handler(ctx({ issueKey: testConfig.issueKey }));
    expect(result.exitCode).toBe(0);
  });

  it('get-issue-dates returns dates', async () => {
    const result = await getIssueDates.handler(ctx({ issueKey: testConfig.issueKey }));
    expect(result.exitCode).toBe(0);
  });

  it('get-project-versions returns versions', async () => {
    const result = await getProjectVersions.handler(ctx({ projectKey: testConfig.projectKey }));
    expect(result.exitCode).toBe(0);
  });

  it('get-project-components returns components', async () => {
    const result = await getProjectComponents.handler(ctx({ projectKey: testConfig.projectKey }));
    expect(result.exitCode).toBe(0);
  });

  it('get-agile-boards returns boards', async () => {
    const result = await getAgileBoards.handler(ctx({ maxResults: 5 }));
    expect(result.exitCode).toBe(0);
  });

  it('get-board-issues returns issues', async () => {
    const result = await getBoardIssues.handler(ctx({ boardId: testConfig.boardId, maxResults: 5 }));
    expect(result.exitCode).toBe(0);
  });

  it('get-sprints-from-board returns sprints', async () => {
    const result = await getSprintsFromBoard.handler(ctx({ boardId: testConfig.boardId, maxResults: 5 }));
    expect(result.exitCode).toBe(0);
  });
});
