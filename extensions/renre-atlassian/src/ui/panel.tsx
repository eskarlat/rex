...
export default function AtlassianPanel({ sdk, extensionName }: Partial<PanelProps>) {
  ...
  const extName = extensionName ?? 'renre-atlassian';
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="rounded-lg border border-border p-6">
        <h2 className="mb-2 text-xl font-semibold">Atlassian</h2>
        <p className="text-sm text-muted-foreground">
          Jira and Confluence integration — 75 CLI commands across 21 domains.
        </p>
        <div className="mt-2 inline-flex items-center gap-1.5">
          <span className="inline-block h-2 w-2 rounded-full bg-[#0052CC]" />
          <span className="text-xs text-muted-foreground">Standard CLI extension</span>
        </div>
      </div>
