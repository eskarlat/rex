...
describe('StatusWidget', () => {
  ...
  it('renders extension name', () => {
    render(<StatusWidget extensionName="playwright-mcp" />);
  ...
  it('calls playwright-mcp:status on button click', async () => {
    const user = userEvent.setup();
    const sdk = createMockSdk();
    sdk.exec.run.mockResolvedValueOnce({
      output: 'playwright-mcp v1.0.0\nStatus: ready',
      exitCode: 0,
    });

    render(<StatusWidget sdk={sdk} extensionName="playwright-mcp" />);
    await user.click(screen.getByRole('button', { name: /check status/i }));

    await waitFor(() => {
      expect(sdk.exec.run).toHaveBeenCalledWith('playwright-mcp:status');
    });
  });
