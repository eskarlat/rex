...
describe('StatusWidget', () => {
  ...
  it('renders extension name', () => {
    render(<StatusWidget extensionName="github-mcp" />);
  ...
  it('calls github-mcp:status on button click', async () => {
    const user = userEvent.setup();
    const sdk = createMockSdk();
    sdk.exec.run.mockResolvedValueOnce({
      output: 'github-mcp v1.0.0\nStatus: ready',
      exitCode: 0,
    });

    render(<StatusWidget sdk={sdk} extensionName="github-mcp" />);
    await user.click(screen.getByRole('button', { name: /check status/i }));

    await waitFor(() => {
      expect(sdk.exec.run).toHaveBeenCalledWith('github-mcp:status');
    });
  });
