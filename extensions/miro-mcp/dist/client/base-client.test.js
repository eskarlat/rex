...
class TestClient extends MiroBaseClient {
    ...
    testRequestRaw(method, path, body) {
        return this.requestRaw(method, path, body);
    }
    testRequestMultipart(method, path, formData) {
        return this.requestMultipart(method, path, formData);
    }
}
...
describe('MiroBaseClient', () => {
    const config = { accessToken: 'test-token-123' };
    let client;
    let fetchMock;
    beforeEach(() => {
    ...
    it('sends Bearer auth header', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith('https://api.miro.com/v2/boards', expect.objectContaining({
    ...
    it('returns parsed JSON response', async () => {
        const result = await client.testRequest('GET', '/boards');
    ...
    it('throws on error responses', async () => {
        ...
        try {
            ...
            expect(err.message).toBe('Miro API error 404: Not found');
        }
    });
    ...
    it('requestRaw throws on error', async () => {
        ...
        try {
            ...
            expect(err).toBeInstanceOf(Error);
            expect(err.message).toBe('Miro API error 500: Server error');
        }
    });
    it('requestMultipart sends FormData without Content-Type', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith('https://api.miro.com/v2/boards/1/images', expect.objectContaining({
            method: 'POST',
            body: formData,
    ...
    it('requestMultipart returns parsed JSON', async () => {
        const formData = new FormData();
        const result = await client.testRequestMultipart('POST', '/boards/1/images', formData);
        expect(result).toEqual({ id: '1' });
    });
});
