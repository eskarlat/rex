...
- Figma MCP server must be connected and accessible
  - Before proceeding, verify the Figma MCP server is connected by checking if Figma MCP tools (e.g., `figma-mcp:get_design_context`) are available.
...
Run `figma-mcp:get_design_context` with the extracted file key and node ID.

```
renre-kit figma-mcp:get_design_context --fileKey ":fileKey" --nodeId "1-2"
```
...
1. Run `renre-kit figma-mcp:get_metadata --fileKey ":fileKey" --nodeId "1-2"` to get the high-level node map
2. Identify the specific child nodes needed from the metadata
3. Fetch individual child nodes with `renre-kit figma-mcp:get_design_context --fileKey ":fileKey" --nodeId ":childNodeId"`

### Step 3: Capture Visual Reference

Run `figma-mcp:get_screenshot` with the same file key and node ID for a visual reference.

```
renre-kit figma-mcp:get_screenshot --fileKey ":fileKey" --nodeId "1-2"
```
...
2. Run `renre-kit figma-mcp:get_design_context --fileKey "kL9xQn2VwM8pYrTb4ZcHjF" --nodeId "42-15"`
3. Run `renre-kit figma-mcp:get_screenshot --fileKey "kL9xQn2VwM8pYrTb4ZcHjF" --nodeId "42-15"` for visual reference
...
2. Run `renre-kit figma-mcp:get_metadata --fileKey "pR8mNv5KqXzGwY2JtCfL4D" --nodeId "10-5"` to understand the page structure
3. Identify main sections from metadata (header, sidebar, content area, cards) and their child node IDs
4. Run `renre-kit figma-mcp:get_design_context --fileKey "pR8mNv5KqXzGwY2JtCfL4D" --nodeId ":childNodeId"` for each major section
5. Run `renre-kit figma-mcp:get_screenshot --fileKey "pR8mNv5KqXzGwY2JtCfL4D" --nodeId "10-5"` for the full page
...
Never implement based on assumptions. Always fetch `figma-mcp:get_design_context` and `figma-mcp:get_screenshot` first.
...
**Solution:** Use `figma-mcp:get_metadata` to get the node structure, then fetch specific nodes individually with `figma-mcp:get_design_context`.
