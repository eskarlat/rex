...
- Figma MCP server must be connected and accessible
  - Before proceeding, verify the Figma MCP server is connected by checking if Figma MCP tools (e.g., `figma-mcp:get_code_connect_suggestions`) are available.
...
Call `figma-mcp:get_code_connect_suggestions` to identify all unmapped components in a single operation. This tool automatically:
...
Parse the URL to extract `fileKey` and `nodeId`, then call `figma-mcp:get_code_connect_suggestions`.
...
```
renre-kit figma-mcp:get_code_connect_suggestions --fileKey ":fileKey" --nodeId "1:2"
```
...
For each unmapped component returned by `figma-mcp:get_code_connect_suggestions`, search the codebase for a matching code component.
...
Once the user confirms their selections, call `figma-mcp:send_code_connect_mappings` with only the accepted mappings. This tool handles batch creation of all mappings in a single call.

**Example:**

```
renre-kit figma-mcp:send_code_connect_mappings --fileKey ":fileKey" --nodeId "1:2" --mappings '[
...
2. Run `renre-kit figma-mcp:get_code_connect_suggestions --fileKey "kL9xQn2VwM8pYrTb4ZcHjF" --nodeId "42:15"`
...
9. Run `renre-kit figma-mcp:send_code_connect_mappings --fileKey "kL9xQn2VwM8pYrTb4ZcHjF" --nodeId "42:15" --mappings '[{ "nodeId": "42:15", "componentName": "Button", "source": "src/components/Button.tsx", "label": "React" }]'`
...
2. Run `renre-kit figma-mcp:get_code_connect_suggestions --fileKey "pR8mNv5KqXzGwY2JtCfL4D" --nodeId "10:50"`
...
7. Run `renre-kit figma-mcp:send_code_connect_mappings --fileKey "pR8mNv5KqXzGwY2JtCfL4D" --nodeId "10:50" --mappings '[{ "nodeId": "10:51", "componentName": "ProductCard", "source": "src/components/ProductCard.tsx", "label": "React" }]'`
...
2. Run `renre-kit figma-mcp:get_code_connect_suggestions --fileKey "8yJDMeWDyBz71EnMOSuUiw" --nodeId "5:20"`
...
9. Run `renre-kit figma-mcp:send_code_connect_mappings --fileKey "8yJDMeWDyBz71EnMOSuUiw" --nodeId "5:20" --mappings '[{ "nodeId": "5:20", "componentName": "CheckIcon", "source": "src/icons/CheckIcon.tsx", "label": "React" }]'`
