...
export function useLogPolling(
  ...
  useEffect(() => {
    ...
    const poll = (): void => {
      void sdk.exec.run('chrome-debugger:console', { format: 'json', limit: 200, ...(levelFilter ? { level: levelFilter } : {}) })
        .then((r) => {
  ...
  useEffect(() => {
    if (!isRunning || paused) return;
    const poll = (): void => {
      void sdk.exec.run('chrome-debugger:network', { format: 'json', limit: 200, ...(methodFilter ? { method: methodFilter } : {}) })
        .then((r) => {
