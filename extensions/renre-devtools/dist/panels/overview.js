...
function useChromeDetection(sdk) {
  ...
  useEffect(() => {
    if (!sdk) return;
    sdk.exec.run("renre-devtools:chrome-check").then((r2) => setChromeCheck(JSON.parse(r2.output))).catch(() => setChromeCheck({ found: false, canInstall: true }));
  }, [sdk]);
  const handleInstall = useCallback(() => {
    if (!sdk) return;
    setInstalling(true);
    void sdk.exec.run("renre-devtools:chrome-install").then((r2) => {
...
function useHeartbeat(sdk, isRunning) {
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      void sdk.exec.run("renre-devtools:heartbeat");
    }, 3e4);
    return () => {
      clearInterval(timer);
    };
...
function useInspect(sdk) {
  ...
  const handleInspect = useCallback(() => {
    ...
    void sdk.exec.run("renre-devtools:inspect").then(() => sdk.exec.run("renre-devtools:selected")).then((r2) => setSelectedElement(r2.output)).catch((e) => sdk.ui.toast({ title: "Inspect failed", description: e instanceof Error ? e.message : String(e) })).finally(() => setInspecting(false));
  }, [sdk]);
