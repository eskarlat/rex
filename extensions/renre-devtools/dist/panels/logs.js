...
function useLogPolling(sdk, isRunning, paused, levelFilter, methodFilter) {
  ...
  useEffect(() => {
    ...
    const poll = () => {
      void sdk.exec.run("renre-devtools:console", { format: "json", limit: 200, ...levelFilter ? { level: levelFilter } : {} }).then((r2) => {
  ...
  useEffect(() => {
    if (!isRunning || paused) return;
    const poll = () => {
      void sdk.exec.run("renre-devtools:network", { format: "json", limit: 200, ...methodFilter ? { method: methodFilter } : {} }).then((r2) => {
        const d = JSON.parse(r2.output);
