...
/** Marketplace mock that includes chrome-debugger as active extension. */
...
/**
 * Set up all common API route mocks with chrome-debugger included.
 */
async function setupDevtoolsAPIMocks(page: Page): Promise<void> {
  await setupAPIMocks(page);
  // Override the marketplace route to include chrome-debugger
  await page.route('**/api/marketplace', (route) => route.fulfill({ json: devtoolsMarketplace }));
}

/**
 * Set up mocks with ONLY chrome-debugger (no other extensions).
...
test.describe('Renre Devtools — Extension Panel Page', () => {
  ...
  test('displays extension name as heading', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'chrome-debugger' })).toBeVisible();
  });
  ...
  test('shows error alert when panel fails to load', async ({ page }) => {
    await page.route('**/api/extensions/chrome-debugger/panels/browser-panel.js', (route) =>
...
test.describe('Renre Devtools — Marketplace', () => {
  ...
  test('shows chrome-debugger in active extensions tab', async ({ page }) => {
    await page.getByRole('tab', { name: /active/i }).click();
    const tabPanel = page.getByRole('tabpanel');
    await expect(tabPanel.getByText('chrome-debugger')).toBeVisible();
  });

  test('shows mcp type badge for chrome-debugger', async ({ page }) => {
    await page.getByRole('tab', { name: /active/i }).click();
    // Scope to the chrome-debugger card area
  ...
  test('has deactivate button for chrome-debugger', async ({ page }) => {
    await page.getByRole('tab', { name: /active/i }).click();
...
test.describe('Renre Devtools — Dashboard Widget', () => {
  ...
  test('widget shows extension name in picker', async ({ page }) => {
    ...
    await expect(pickerList.getByText('chrome-debugger')).toBeVisible();
  });
...
test.describe('Renre Devtools — Install Flow', () => {
  test('install from available → appears in installed tab', async ({ page }) => {
    ...
    await expect(page.getByText('chrome-debugger')).toBeVisible();
