...
test.describe('Renre Devtools — dashboard layout API', () => {
  test('can save layout with renre-devtools widget', async ({ request }) => {
    const layout = {
      widgets: [
        {
    ...
    expect(data.widgets[0].id).toBe('renre-devtools:browser-widget');
    expect(data.widgets[0].extensionName).toBe('renre-devtools');
    expect(data.widgets[0].size).toEqual({ w: 4, h: 2 });
  });

  test('can save layout with multiple widgets including renre-devtools', async ({ request }) => {
    const layout = {
      widgets: [
        {
    ...
    expect(data.widgets[1].extensionName).toBe('renre-devtools');
  });
});
